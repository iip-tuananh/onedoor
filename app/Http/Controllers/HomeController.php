...
class HomeController extends Controller
{
    public function home()
    {
        $data['hotnews'] = Blog::where([
            ...
            ['status','=',1]
        ])->with('cate')->orderBy('id','DESC')->limit(7)->get(['id','title','slug','created_at','image','description','category']);
        ...
        $data['categoryhomehot'] = Category::where('status',1)
            ->with(['product' => function($query) {
                $query->where(['status'=>1,'home_status'=>1])
                    ->orderBy('id','DESC')
