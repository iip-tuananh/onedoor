...
class ProductController extends Controller
{
    ...
    {
        $data['list'] = Product::with(['thuonghieu:id,name'])
            ->where('status', 1)
            ->orderBy('id', 'DESC')
            ->select('id', 'category', 'name', 'discount', 'price', 'images', 'slug', 'cate_slug', 'type_slug', 'status_variant', 'variant', 'size', 'preserve', 'species', 'thuonghieu_id', 'tags')
            ->paginate(24);
        $data['filter'] = TagCate::with(['tags' => function($query) {
            $query->where('status', 1);
        }])->where('status', 1)->get();
        $data['thuonghieu'] = Partner::where('status', 1)->orderBy('id', 'ASC')->get(['id', 'name', 'image', 'link']);
        $data['cate_slug'] = "";
        $data['type_slug'] = "";
        $data['type_two_slug'] = "";
        $data['title'] = 'Tất cả sản phẩm';
        $data['content'] = 'none';
        return view('product.tatcasp', $data);
    }
    public function detail_product($cate, $type, $id)
    {
        ...
        // Thêm dữ liệu khuyến mãi từ preserve
        if (!empty($data['product']->preserve)) {
            $data['khuyenmai'] = json_decode($data['product']->preserve, true);
        } else {
            $data['khuyenmai'] = [];
        }
        ...
        if (!empty($data['product']->images)) {
            $data['img'] = json_decode($data['product']->images, true);
        } else {
            $data['img'] = [];
        }
        
        // Tính phần trăm giảm giá
        if ($data['product']->discount > 0 && $data['product']->price > 0) {
            $data['phantram'] = (($data['product']->price - $data['product']->discount) / $data['product']->price) * 100;
        } else {
            $data['phantram'] = 0;
        }
    ...
    {
        $data['list'] = Product::with(['thuonghieu:id,name'])
            ->where(['status' => 1, 'cate_slug' => $danhmuc])
            ->orderBy('id', 'DESC')
            ->select('id', 'category', 'name', 'discount', 'price', 'images', 'slug', 'cate_slug', 'type_slug', 'description', 'status_variant', 'variant', 'size', 'preserve', 'tags', 'thuonghieu_id')
            ->paginate(20);
        $data['thuonghieu'] = Partner::where('status', 1)->orderBy('id', 'ASC')->get(['id', 'name', 'image', 'link']);
        ...
        if ($data['cateno']) {
            $data['filter'] = TagCate::with(['tags' => function($query) {
                $query->where('status', 1);
            }])->where('status', 1)->where('cate_product_id', $data['cateno']->id)->get();
            ...
            $data['filter'] = collect();
            $data['cate_slug'] = "";
            $data['title'] = "Danh mục không tồn tại";
            $data['content'] = "";
        }
        $data['type_slug'] = "";
        $data['type_two_slug'] = "";
    ...
    {
        $data['list'] = Product::with(['thuonghieu:id,name'])
            ->where(['status' => 1, 'cate_slug' => $danhmuc, 'type_slug' => $loaidanhmuc])
            ->orderBy('id', 'DESC')
            ->select('id', 'category', 'name', 'discount', 'price', 'images', 'slug', 'cate_slug', 'type_slug', 'description', 'status_variant', 'variant', 'size', 'preserve', 'tags', 'thuonghieu_id')
            ->paginate(20);
        $data['thuonghieu'] = Partner::where('status', 1)->orderBy('id', 'ASC')->get(['id', 'name', 'image', 'link']);
        ...
        if ($data['type']) {
            ...
            $data['type_slug'] = "";
            $data['title'] = "Loại sản phẩm không tồn tại";
            $data['content'] = "";
        }
    ...
    {
        $data['list'] = Product::with(['thuonghieu:id,name'])
            ->where(['status' => 1, 'cate_slug' => $danhmuc, 'type_slug' => $loaidanhmuc, 'type_two_slug' => $thuonghieu])
            ->orderBy('id', 'DESC')
            ->select('id', 'category', 'name', 'discount', 'price', 'images', 'slug', 'cate_slug', 'type_slug', 'description', 'status_variant', 'variant', 'size', 'preserve', 'tags', 'thuonghieu_id')
            ->paginate(20);
        $data['thuonghieu'] = Partner::where('status', 1)->orderBy('id', 'ASC')->get(['id', 'name', 'image', 'link']);
